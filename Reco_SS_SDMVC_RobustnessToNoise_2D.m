...
noise_level_vec = [40,30,20,-35];

%%
t1 = tic;
OriDataPath = '.\data\DataGeneration_SS_SDMVC_RobustnessToGBp_2D\delta_0_0_0\';
PHPath = [OriDataPath,'PH\'];
DataPath = '.\data\DataGeneration_SS_SDMVC_RobustnessToNoise_2D\';
if ~exist(DataPath,'dir');mkdir(DataPath);end                 	% 确保创建图片存储路径
...
for count_file = 1:numel(noise_level_vec)
    noise_level = noise_level_vec(count_file);
    
    FigPath = [DataPath,num2str(noise_level),'dB\'];
    ...
    for j = 1:numel(lambda_vec)   
        ...
        opts.SNR = 3;
        opts.tol = 1e-3;
        [c_reco,opts] = TSD_FISTA(Sz_reco,Uz_reco,dz_reco,Phi_reco,lambda,opts);% sFISTA
        ...
        fprintf('Progress1 = %% %f\t Progress2 = %% %f\t toc = %f\n',...
            100*count_file/numel(noise_level_vec),100*j/numel(lambda_vec),toc(t1));% 进度
