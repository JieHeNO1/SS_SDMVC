...
[xxx0,yyy0,zzz0] = ndgrid(x0,y0,z);                                 % 网格剖分
...
[xxx,yyy,zzz] = ndgrid(x,y,z);                                      % 正交坐标系网格剖分

%% calibration sample 1mm x 1mm x 1mm
dx = gpuArray(0);
dy = gpuArray(0);
dz = gpuArray(0);
...
for count_file = 1:size(delta_mat,1)
    ...
    t2 = tic;
    for iz = 1:Nz
        Sz_2D = zeros(Nf*Na,Nx0*Ny0*Nz);                                    % 创建Sz_2D
        for jxyz = 1:Nx0*Ny0*Nz                                             % 采样半径
            dxxx = gpuArray(xxx0(jxyz))+ddx;
            dyyy = gpuArray(yyy0(jxyz))+ddy;
            dzzz = gpuArray(zzz0(jxyz))+ddz;
            for ia = 1:Na                                                   % FFL角度
                [a_jr,r_jr] = cart2pol(dxxx,dyyy);
                ...
                Ba_P = repmat((G*G_ratio).*(z(iz)-dzzz),[Nt,1]);            % 中心平面的平面场强，正方向为a(j1)+pi/2
                ...
                Sz_2D(idx_row,jxyz) = mean(Uz_F_r(F_Vec_Idx,:),2);          % 单次测量数据填充
            end
        end
        Sz_2D = gather(Sz_2D);
    ...
    for iz = 1:Nz
        ...
        Sz_2D_cubic = zeros(Nf*Na,Nx*Ny*Nz);                                % 网格插值平面系统矩阵，180度角度数据删除
        for i = 1:Nf*Na                                                     % 网格插值系统矩阵行索引
            F = scatteredInterpolant(xxx0(:),yyy0(:),zzz0(:),Sz_2D(i,:).'); % 生成插值函数
            Sz_2D_cubic(i,:) = F(xxx(:),yyy(:),zzz(:));                            % 生成插值：正交坐标系平面系统矩阵
        end
    ...
    fprintf('\nBigProgress = %% %f\t toc = %f\n\n',100*count_file/size(delta_mat,1),toc(t1));      % 进度
end